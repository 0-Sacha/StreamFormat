...
#define PCT_TEST_FUNC_LowBufferSize(k)                                         \
  PCT_TEST_FUNC(LIMITS, LowBufferSize##k) {                                    \
    char buffer[k] = {0};                                                      \
    StreamFormat::FMT::FormatInChar(buffer, "0123456789").value();             \
    for (int i = 0; i < k; ++i)                                                \
      PCT_EQ(buffer[i] - '0', i);                                              \
  }
...
PCT_TEST_FUNC(LIMITS, PlainLowBufferSize1) {
  char buffer[1] = {0};
  StreamFormat::FMT::FormatInChar(buffer, "0").value();
  for (int i = 0; i < 1; ++i)
    PCT_EQ(buffer[i] - '0', i);
}
...
#define PCT_TEST_FUNC_LowBufferSizeArray(k)                                       \
  PCT_TEST_FUNC(LIMITS, LowBufferSizeArray##k) {                                  \
    ...
    StreamFormat::FMT::Detail::FormatInManager(manager, false, format).value();   \
    for (int i = 0; i < k; ++i)                                                   \
      PCT_EQ(buffer[i] - '0', i);                                                 \
  }
...
PCT_TEST_FUNC(LIMITS, PlainLowBufferSizeArray10) {
  ...
  StreamFormat::FMT::Detail::FormatInManager(manager, false, fmt).value();
  for (int i = 0; i < 10; ++i)
    PCT_EQ(buffer[i] - '0', i);
}
...
PCT_TEST_FUNC(LIMITS, LowBufferSizeArray5Number9) {
  ...
  StreamFormat::FMT::Detail::FormatInManager(manager, false, fmt, i).value();
  for (int k = 0; k < 10; ++k)
