...
namespace ProjectCore::FLog::Detail
{
    ...
    public:
        ...
        void Log(const SeverityValueType& severity, const Format& format, Args&& ...args)
        {
            ...
            FMT::Detail::FormatInBufferOutManager(preFormatBufferOutManager, preFormatBufferInProperties, false, FORMAT_SV("time", logTime), FORMAT_SV("name", m_Name), FORMAT_SV("data", FLog::AddIndentInFormat(format)));
        ...
        void Log(const SeverityValueType& severity, T&& t)
        {
            ...
            FMT::Detail::FormatInBufferOutManager(fullFormatBufferOutManager, fullFormatBufferInProperties, true, FORMAT_SV("data", t), FORMAT_SV("color", severity), FORMAT_SV("time", logTime), FORMAT_SV("name", m_Name));
            m_Stream.write(fullFormatBufferOutManager.GetBuffer(), static_cast<std::streamsize>(fullFormatBufferOutManager.GetLastGeneratedDataSize()));
            m_Stream.flush();
        }
    };
