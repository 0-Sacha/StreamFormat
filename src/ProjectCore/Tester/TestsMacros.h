...
namespace ProjectCore::Test
{
    ...
    public:
        ...
        {
            static FLog::BasicLogger instance("ProjectCore-Test", "{name} {color} -> {data}");
            return instance;
        }
    };
}
...
// Assert log if false
#define PROJECTCORE_TEST_ASSERT(x) \
    if (!(x)) PROJECTCORE_TEST_FAIL("ASSERT FAIL : {0:C:red}", #x)

// Check if false
#define PROJECTCORE_TEST_TEST(x)                         \
    if (x)                                               \
        PROJECTCORE_TEST_OK("TEST OK : {:C:green}", #x); \
    else                                                 \
        PROJECTCORE_TEST_FAIL("TEST FAIL : {:C:red}", #x)

// Check if equal with file printed
#define PROJECTCORE_TEST_EQ(x, y)                         \
    if (x == y)                                           \
        PROJECTCORE_TEST_OK("{C:green}{} == {}", #x, #y); \
    else                                                  \
        PROJECTCORE_TEST_FAIL("{C:red}{} == {}", #x, #y)

// Check if not equal with file printed
#define PROJECTCORE_TEST_NEQ(x, y)                        \
    if (x != y)                                           \
        PROJECTCORE_TEST_OK("{C:green}{} != {}", #x, #y); \
    else                                                  \
        PROJECTCORE_TEST_FAIL("{C:red}{} != {}", #x, #y)
...
#define PROJECTCORE_TEST_FN(fn, resExpected)                                                   \
    {                                                                                          \
        auto res = fn;                                                                         \
        if (res == resExpected)                                                                \
            PROJECTCORE_TEST_OK("{C:green}{} return {}", #fn, res);                            \
        else                                                                                   \
            PROJECTCORE_TEST_FAIL("{C:red}{} return {} instead of {}", #fn, res, resExpected); \
    }
