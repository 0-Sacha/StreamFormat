...
namespace ProjectCore::FMT::Detail
{
    ...
    {
        if (CanMoveForward(sizeToCopy) == false) return FastReadCharPtr(str, GetBufferRemainingSize(), isZeroEnded);
    ...
    template <typename CharBuffer>
    template <typename CharPtr, typename CharPattern>
    bool BasicBufferIn<CharBuffer>::FastReadCharPtrRegex([[maybe_unused]] std::basic_string_view<CharPattern> regexPattern, [[maybe_unused]] CharPtr* str,
                                                         [[maybe_unused]] std::size_t sizeToCopy, [[maybe_unused]] bool isZeroEnded)
    {
        throw FMTImplError{};
    }
}
