...
namespace ProjectCore::FMT
{
    ...
    namespace Detail
    {
        ...
        requires(IsCharType<CharFormat>::Value&& IsCharType<CharBuffer>::Value) void FormatInBufferOutManager(Detail::BasicBufferOutManager<CharBuffer>&    bufferOutManager,
                                                                                                              const Detail::BufferInProperties<CharFormat>& bufferInProperties,
                                                                                                              bool                                          newline, Args&&... args)
        {
            ...
            if (newline) context.BufferOut().PushBack('\n');
            context.Terminate();
        }
        ...
        template <typename CharFormat = char, typename CharBuffer = CharFormat, typename... Args>
        requires(IsCharType<CharFormat>::Value&& IsCharType<CharBuffer>::Value) std::shared_ptr<Detail::BasicBufferOutManager<CharBuffer>> FormatAndGetBufferOut(
            Detail::BufferInProperties<CharFormat>& bufferInProperties, Args&&... args)
        {
    ...
    requires(Detail::CanBeUseForFMTBufferIn<Format>&& Detail::IsCharType<CharBuffer>::Value) void FormatInChar(CharBuffer (&buffer)[BUFFER_SIZE], const Format& format,
                                                                                                               Args&&... args)
    {
    ...
    requires(Detail::CanBeUseForFMTBufferIn<Format>&& Detail::IsCharType<CharBuffer>::Value) void FormatInChar(CharBuffer const buffer, const std::size_t bufferSize,
    ...
    requires(Detail::CanBeUseForFMTBufferIn<Format>&& Detail::IsCharType<CharBuffer>::Value) void CFilePrint(FILE* stream, const Format& format, Args&&... args)
    {
    ...
    template <typename Format = std::string_view, typename CharBuffer = typename Detail::FMTCharTypeFromBuffer<Format>::Type, typename... Args>
    requires(Detail::CanBeUseForFMTBufferIn<Format>&& Detail::IsCharType<CharBuffer>::Value) void CFilePrintLn(FILE* stream, const Format& format, Args&&... args)
    {
        Detail::DynamicBufferOutManager<CharBuffer> bufferOutManager(256);
        Detail::BufferInProperties                  bufferInProperties(format);
    ...
    template <typename Format = std::string_view, typename CharBuffer = typename Detail::FMTCharTypeFromBuffer<Format>::Type, typename... Args>
    requires(Detail::CanBeUseForFMTBufferIn<Format>&& Detail::IsCharType<CharBuffer>::Value) void FilePrint(std::basic_ostream<CharBuffer>& stream, const Format& format,
                                                                                                            Args&&... args)
    {
        Detail::DynamicBufferOutManager<CharBuffer> bufferOutManager(256);
    ...
    template <typename Format = std::string_view, typename CharBuffer = typename Detail::FMTCharTypeFromBuffer<Format>::Type, typename... Args>
    requires(Detail::CanBeUseForFMTBufferIn<Format>&& Detail::IsCharType<CharBuffer>::Value) void FilePrintLn(std::basic_ostream<CharBuffer>& stream, const Format& format,
                                                                                                              Args&&... args)
    {
        Detail::DynamicBufferOutManager<CharBuffer> bufferOutManager(256);
        Detail::BufferInProperties                  bufferInProperties(format);
    ...
    template <typename Format = std::string_view, typename CharBuffer, typename... Args>
    requires(Detail::CanBeUseForFMTBufferIn<Format>&& Detail::IsCharType<CharBuffer>::Value) void FormatInString(std::basic_string<CharBuffer>& str, const Format& format,
                                                                                                                 Args&&... args)
    {
        Detail::DynamicBufferOutManager<CharBuffer> bufferOutManager(256);
        Detail::BufferInProperties                  bufferInProperties(format);
    ...
    template <typename Format = std::string_view, typename CharBuffer = typename Detail::FMTCharTypeFromBuffer<Format>::Type, typename... Args>
    requires(Detail::CanBeUseForFMTBufferIn<Format>&& Detail::IsCharType<CharBuffer>::Value) inline std::basic_string<CharBuffer> FormatString(const Format& format, Args&&... args)
    {
        Detail::DynamicBufferOutManager<CharBuffer> bufferOutManager(256);
        Detail::BufferInProperties                  bufferInProperties(format);
        Detail::FormatInBufferOutManager(bufferOutManager, bufferInProperties, false, std::forward<Args>(args)...);
    ...
    requires(Detail::IsCharType<CharBuffer>::Value) void FormatInChar(CharBuffer (&buffer)[BUFFER_SIZE], T&& t)
    {
    ...
    requires(Detail::IsCharType<CharBuffer>::Value) void FormatInChar(CharBuffer* const buffer, const std::size_t bufferSize, T&& t)
    {
        Detail::GivenBufferOutManager<CharBuffer> bufferOutManager(buffer, bufferSize);
        Detail::FormatInBufferOutManager(bufferOutManager, false, std::forward<T>(t));
    }
    ...
    template <typename CharBuffer = char, typename T>
    requires(Detail::IsCharType<CharBuffer>::Value) void CFilePrint(FILE* stream, T&& t)
    {
    ...
    template <typename CharBuffer = char, typename T>
    requires(Detail::IsCharType<CharBuffer>::Value) void CFilePrintLn(FILE* stream, T&& t)
    {
        Detail::DynamicBufferOutManager<CharBuffer> bufferOutManager(32);
    ...
    template <typename CharBuffer = char, typename T>
    requires(Detail::IsCharType<CharBuffer>::Value) void FilePrint(std::basic_ostream<CharBuffer>& stream, T&& t)
    {
        Detail::DynamicBufferOutManager<CharBuffer> bufferOutManager(32);
        Detail::FormatInBufferOutManager(bufferOutManager, false, std::forward<T>(t));
    ...
    template <typename CharBuffer = char, typename T>
    requires(Detail::IsCharType<CharBuffer>::Value) void FilePrintLn(std::basic_ostream<CharBuffer>& stream, T&& t)
    {
        Detail::DynamicBufferOutManager<CharBuffer> bufferOutManager(32);
        Detail::FormatInBufferOutManager(bufferOutManager, true, std::forward<T>(t));
    ...
    template <typename CharBuffer = char, typename T>
    requires(Detail::IsCharType<CharBuffer>::Value) void FormatInString(std::basic_string<CharBuffer>& str, T&& t)
    {
        Detail::DynamicBufferOutManager<CharBuffer> bufferOutManager(32);
        Detail::FormatInBufferOutManager(bufferOutManager, false, std::forward<T>(t));
        str = bufferOutManager.GetLastGeneratedString();
    ...
    template <typename CharBuffer = char, typename T>
    requires(Detail::IsCharType<CharBuffer>::Value) inline std::basic_string<CharBuffer> FormatString(T&& t)
    {
        Detail::DynamicBufferOutManager<CharBuffer> bufferOutManager(32);
        Detail::FormatInBufferOutManager(bufferOutManager, false, std::forward<T>(t));
        return bufferOutManager.GetLastGeneratedString();
