...
namespace ProjectCore::FMT::Detail
{
    ...
    {
        if (target.Intensity != m_CurrentContextProperties.Style.Intensity) { ApplyStyle(target.Intensity);     m_CurrentContextProperties.Style.Intensity = target.Intensity;  }
        if (target.Italic != m_CurrentContextProperties.Style.Italic)       { ApplyStyle(target.Italic);        m_CurrentContextProperties.Style.Italic = target.Italic;        }
        if (target.Blink != m_CurrentContextProperties.Style.Blink)         { ApplyStyle(target.Blink);         m_CurrentContextProperties.Style.Blink = target.Blink;          }
        if (target.Inverted != m_CurrentContextProperties.Style.Inverted)   { ApplyStyle(target.Inverted);      m_CurrentContextProperties.Style.Inverted = target.Inverted;    }
        if (target.Ideogram != m_CurrentContextProperties.Style.Ideogram)   { ApplyStyle(target.Ideogram);      m_CurrentContextProperties.Style.Ideogram = target.Ideogram;    }
        if (target.Script != m_CurrentContextProperties.Style.Script)       { ApplyStyle(target.Script);        m_CurrentContextProperties.Style.Script = target.Script;        }

        if (target.Underline != m_CurrentContextProperties.Style.Underline) { ApplyStyle(target.Underline);     m_CurrentContextProperties.Style.Underline = target.Underline;  }
