...
namespace stream::fmt::buf
{
    ...
    public:
        template <typename T>
        [[nodiscard]] std::expected<void, FMTResult> write_integer_h(T i, std::uint8_t digitSize, const TChar* const lut, TChar base_prefix = '\0', detail::ShiftInfo shift = detail::ShiftInfo{})
        {
    ...
    public:
        ...
        [[nodiscard]] std::expected<void, FMTResult> write_float_formatdata(T i, const detail::FormatData<TChar>& formatdata)
        {
            ...
            {
                if (formatdata.shift.type != detail::ShiftInfo::ShiftType::Nothing)
                    return write_float(i, formatdata.float_precision, formatdata.shift);
            }
    ...
    public:
        ...
        {
            if (shift.size <= 0)
                return WriteManip(buffer).fast_write_char_array(str, size);
