...
namespace stream::fmt::buf
{
    ...
    protected:
        StreamIO(BasicStreamIOManager<CharType>& ostream_manager) noexcept
            : Stream<CharType>()
            , Manager(ostream_manager)
        {}
    ...
    public:
        inline void set(const TChar c) noexcept { *current_pos = c; }
    ...
    template <typename T>
    concept convertible_to_streamio_info = requires(T&& t)
    {
        StreamIO(std::forward<T>(t));
    };
}
