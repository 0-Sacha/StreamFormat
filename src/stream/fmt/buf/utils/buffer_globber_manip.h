...
namespace stream::fmt::buf
{
    ...
    private:
        ...
        {
            if (Access(glob).is_end_of_string()) { return istream.current_pos; }

            if (Access(istream).is_end_of_string()) { return nullptr; }
    ...
    public:
        [[nodiscard]] static std::expected<void, FMTResult> buffer_exec_glob(Stream<TChar>& istream, Stream<TChar>& glob, [[maybe_unused]] PatternMatchType patternMatchtype = PatternMatchType::MatchBiggest)
        {
            const TChar* furtherPointMatched = SF_TRY(buffer_exec_glob_(istream, glob));
            if (furtherPointMatched != nullptr)
                istream.current_pos = furtherPointMatched;
            return {};
        }
    };
