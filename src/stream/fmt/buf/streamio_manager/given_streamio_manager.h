...
namespace stream::fmt::buf
{
    ...
    public:
        ...
        GivenStreamIOManager(CharType (&buffer)[SIZE])
            : buffer_(buffer)
            , buffer_size_(SIZE)
        {}

        GivenStreamIOManager(CharType* buffer, std::size_t buffer_size)
            : buffer_(buffer)
    ...
    public:
        CharType*       get_buffer() override { return buffer_; }
        const CharType* get_buffer() const override { return buffer_; }
        std::size_t     get_buffer_size() const override { return buffer_size_; }
