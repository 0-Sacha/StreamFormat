...
namespace stream::fmt::buf
{
    ...
    protected:
        [[nodiscard]] virtual std::expected<void, FMTResult> begin_context_impl() { return {}; }
        virtual void compute_generated_size_impl(const std::size_t /* totalGeneratedLength */) {}

    public:
        [[nodiscard]] std::expected<void, FMTResult> BeginContext() { return begin_context_impl(); }
    ...
    public:
        StreamView<CharType> get_last_generated_buffer_info_view() const { return StreamView<CharType>(get_buffer(), m_LastGeneratedDataSize); }
        operator StreamView<CharType>() const { return get_last_generated_buffer_info_view(); }
        std::basic_string_view<CharType> get_last_generated_string_view() const { return std::basic_string_view<CharType>(get_buffer(), m_LastGeneratedDataSize); }
        operator std::basic_string_view<CharType>() const { return get_last_generated_string_view(); }

    public:
        std::size_t get_last_generated_data_size() const { return m_LastGeneratedDataSize; }

    private:
        void set_last_generated_data_size(const std::size_t size) { m_LastGeneratedDataSize = size; }
