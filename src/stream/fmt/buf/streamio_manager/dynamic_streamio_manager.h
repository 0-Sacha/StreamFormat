...
namespace stream::fmt::buf
{
    ...
    {
    public:
        DynamicStreamIOManager(std::size_t beginSize = DEFAULT_BEGIN_SIZE)
            : buffer_size_(beginSize)
        {}
    ...
    public:
        CharType*       get_buffer() override { return buffer_.get(); }
        const CharType* get_buffer() const override { return buffer_.get(); }
        std::size_t     get_buffer_size() const override { if (buffer_ == nullptr) return 0; return buffer_size_; }
    ...
    public:
        ...
        [[nodiscard]] std::expected<void, FMTResult> add_size(const std::size_t count) override { return resize(count + buffer_size_); }
    ...
    public:
        ShrinkDynamicStreamIOManager(std::size_t beginSize = DEFAULT_BEGIN_SIZE)
            : Base(beginSize)
    ...
    public:
        ...
        {
            if (buffer_size_ > static_cast<std::size_t>(m_MeanGeneratedSize * MEAN_SIZE_OVERFLOW))
