...
namespace stream::fmt::buf
{
    ...
    protected:
        FMTStreamIO(BasicStreamIOManager<CharType>& ostream_manager) noexcept
            : StreamIO<CharType>(ostream_manager)
        {}
    ...
    public:
        constexpr inline void add_no_stride(const std::size_t no_stride) noexcept { buffer.NoStride += no_stride; }

        constexpr inline void add_indent(const std::size_t indent) noexcept { buffer.indent += indent; }
        constexpr inline void remove_indent(const std::size_t indent) noexcept { buffer.indent -= indent; }
        constexpr inline void set_indent() noexcept { buffer.indent = Manip(buffer).get_buffer_current_size() - buffer.NoStride; }
