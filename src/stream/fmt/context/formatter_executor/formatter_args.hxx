...
namespace stream::fmt::detail {
    ...
    public:
        FormatterArgs(const TypeWithoutRef& t, Rest&&... rest) : FormatterArgs<Rest...>(std::forward<Rest>(rest)...), m_Value(t) {}
    ...
    public:
        [[nodiscard]] inline std::expected<PointerID, FMTResult> get_pointerid_at(std::int32_t idx) {
            if (idx == 0) return PointerID{.type_info = typeid(const TypeWithoutRef), .ptr = const_cast<void*>(static_cast<const void*>(&m_Value))};
