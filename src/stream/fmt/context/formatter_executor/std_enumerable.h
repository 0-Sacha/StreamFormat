...
namespace stream::fmt {
    ...
    public:
        explicit inline STDEnumerable(const T& value, std::basic_string_view<CharJoin> strJoin = ", ", std::basic_string_view<CharBegin> strBegin = "{ ",
                                      std::basic_string_view<CharEnd> strEnd = " }", const std::int32_t beginIdx = 0, const std::int32_t size = -1)
            : m_Value(value), m_StrJoin(strJoin), m_StrBegin(strBegin), m_StrEnd(strEnd), m_BeginIdx(beginIdx), m_Size(size < 0 ? (std::int32_t)value.size() - beginIdx : size) {}
