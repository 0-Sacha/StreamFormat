...
namespace stream::fmt::detail {
    ...
    template <typename From>
        requires(std::is_convertible_v<From, typename std::int32_t> && std::is_convertible_v<const From&, typename std::int32_t>)
    struct FMTContextConvert<std::int32_t, From> {
        static constexpr bool         is_convertible = true;
        static constexpr std::int32_t convert(const From& from) {
            return std::int32_t(static_cast<typename std::int32_t>(from));
        }
    };
    ...
    template <typename From, typename To>
        requires(std::is_same_v<detail::get_base_type<From>, detail::get_base_type<To>>)
