            ...
            for (auto& sink : sinks_) {
                if (sink->need_to_log(severity)) {
                    ...
                    auto format_pattern_str =
                        fmt::detail::format_in_manager(manager_pattern, false, std::string_view(sink->get_pattern(severity)), FORMAT_SV("logtime", log_time),
                                                       FORMAT_SV("name", FutureConcatNameAndSinkName(name_)), FORMAT_SV("data", flog::AddIndentInFormat(format)));
                    auto format_format_str = fmt::detail::format_in_manager(manager_format, false, static_cast<std::string_view>(*format_pattern_str), std::forward<Args>(args)...,
                                                                            FORMAT_SV("sink", sink->get_name()), FORMAT_SV("color", severity));
                    sink->WriteToSink(static_cast<std::basic_string_view<CharType>>(*format_format_str));
                }
            }
        ...
        void log(const SeverityValueType& severity, T&& t) {
            ...
            for (auto& sink : sinks_) {
                if (sink->need_to_log(severity)) {
                    ...
                    auto format_buffer = fmt::detail::format_in_manager(manager, false, std::string_view(sink->get_pattern(severity)), FORMAT_SV("logtime", log_time),
                                                                        FORMAT_SV("name", ConcatNameAndSinkName(name_, sink->get_name())), FORMAT_SV("data", t));
                    sink->WriteToSink(static_cast<std::basic_string_view<CharType>>(*format_buffer));
                }
            }
