...
namespace StreamFormat::FMT
{
    ...
    {
        [[nodiscard]] static inline std::expected<void, FMTResult> Format(const std::map<T1, T2>& t, FormatterExecutor& executor) { FormatterType<ForwardAsSTDEnumerable<std::map<T1, T2>>, FormatterExecutor>::Format(t, executor); }
    };
