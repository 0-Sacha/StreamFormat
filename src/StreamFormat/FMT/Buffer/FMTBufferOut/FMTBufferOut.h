...
namespace StreamFormat::FMT::Detail
{
    ...
    public:
        ...
        inline void        SetIndent() noexcept { m_Indent = GetBufferCurrentSize() - GetNoStride(); }
    ...
    public:
        template <typename T>
        void WriteInt(T i, ShiftType st = ShiftType::Default, ShiftSize shift = ShiftSize{}, ShiftPrint sp = ShiftPrint{});
        template <typename T>
        void WriteUInt(T i, ShiftType st = ShiftType::Default, ShiftSize shift = ShiftSize{}, ShiftPrint sp = ShiftPrint{});
        template <typename T>
        void WriteFloat(T i, FloatPrecision floatPrecision = FloatPrecision{}, ShiftType st = ShiftType::Nothing, ShiftSize shift = ShiftSize{}, ShiftPrint sp = ShiftPrint{});

        template <typename T>
        void WriteIntAsBin(T i, DigitSize digitSize = DigitSize{}, ShiftType st = ShiftType::Nothing, ShiftSize shift = ShiftSize{}, ShiftPrint sp = ShiftPrint{},
                           bool prefix = true);
        template <typename T>
        void WriteIntAsHex(T i, DigitSize digitSize = DigitSize{}, ShiftType st = ShiftType::Nothing, ShiftSize shift = ShiftSize{}, ShiftPrint sp = ShiftPrint{},
                           bool prefix = true, Detail::PrintStyle uppercase = PrintStyle::Nothing);
        template <typename T>
        void WriteIntAsOct(T i, DigitSize digitSize = DigitSize{}, ShiftType st = ShiftType::Nothing, ShiftSize shift = ShiftSize{}, ShiftPrint sp = ShiftPrint{},
                           bool prefix = true);

    public:
        template <typename T, typename FormatDataCharBuffer>
        void WriteIntFormatData(T i, const FormatData<FormatDataCharBuffer>& formatData);
        template <typename T, typename FormatDataCharBuffer>
        void WriteUIntFormatData(T i, const FormatData<FormatDataCharBuffer>& formatData);
        template <typename T, typename FormatDataCharBuffer>
        void WriteFloatFormatData(T i, const FormatData<FormatDataCharBuffer>& formatData);
    ...
    public:
        ...
        template <typename CharStr>
        inline void WriteCharPtrNSize(const CharStr* str, ShiftType st = ShiftType::Default, ShiftSize shift = ShiftSize{}, ShiftPrint sp = ShiftPrint{})
        {
            WriteStringView(std::basic_string_view<CharStr>(str), st, shift, sp);
        }
        template <typename CharStr, std::size_t SIZE>
        inline void WriteCharArray(const CharStr (&str)[SIZE], ShiftType st = ShiftType::Default, ShiftSize shift = ShiftSize{}, ShiftPrint sp = ShiftPrint{})
        {
            WriteCharPtr(str, str[SIZE - 1] == 0 ? SIZE - 1 : SIZE, st, shift, sp);
        }
        template <typename CharStr>
        inline void WriteCharBound(const CharStr* begin, const CharStr* end, ShiftType st = ShiftType::Default, ShiftSize shift = ShiftSize{}, ShiftPrint sp = ShiftPrint{})
        {
            WriteCharPtr(begin, end - begin, st, shift, sp);
        }
        template <typename CharStr>
        inline void WriteStringView(const std::basic_string_view<CharStr>& str, ShiftType st = ShiftType::Default, ShiftSize shift = ShiftSize{}, ShiftPrint sp = ShiftPrint{})
        {
            WriteCharPtr(str.data(), str.size(), st, shift, sp);
        }
        template <typename CharStr>
        inline void WriteString(const std::basic_string<CharStr>& str, ShiftType st = ShiftType::Default, ShiftSize shift = ShiftSize{}, ShiftPrint sp = ShiftPrint{})
        {
            WriteCharPtr(str.data(), str.size(), st, shift, sp);
        }
    ...
    public:
        ...
        inline void SetCharCheckIndent(const CharBuffer c)
        {
            SetChar(c);
            if (c == '\n') PushBack(' ', m_Indent);
        }
        inline void PushBackCheckIndent(const CharBuffer c)
        {
            PushBack(c);
            if (c == '\n') PushBack(' ', m_Indent);
        }
        ...
        template <typename CharStr>
        inline void WriteIndentCharPtrNSize(const CharStr* str)
        {
            WriteIndentStringView(std::basic_string_view<CharStr>(str));
        }
        template <typename CharStr, std::size_t SIZE>
        inline void WriteIndentCharArray(const CharStr (&str)[SIZE])
        {
            WriteIndentCharPtr(str, str[SIZE - 1] == 0 ? SIZE - 1 : SIZE);
        }
        template <typename CharStr>
        ...
        template <typename CharStr>
        inline void WriteIndentStringView(const std::basic_string_view<CharStr>& str)
        {
            WriteIndentCharPtr(str.data(), str.size());
        }
        template <typename CharStr>
        inline void WriteIndentString(const std::basic_string<CharStr>& str)
        {
            WriteIndentCharPtr(str.data(), str.size());
        }
    ...
    public:
        ...
        inline void PrintShiftCenterBegin(const Detail::ShiftType st, const Detail::ShiftPrint sp, T& shift)
        {
            if (st == Detail::ShiftType::CenterRight || st == Detail::ShiftType::CenterLeft)
            {
                ...
                if (st == Detail::ShiftType::CenterLeft) shift_ = (shift + 1) / 2;
        ...
        {
            if (st == Detail::ShiftType::CenterRight || st == Detail::ShiftType::CenterLeft) PushBack(sp.After, shift);
        }

        template <typename T>
        inline void PrintShiftRightAll(const Detail::ShiftType st, const Detail::ShiftPrint sp, const T shift)
        {
            if (st == Detail::ShiftType::Right) PushBack(sp.Before, shift);
        }

        template <typename T>
        inline void PrintShiftLeftAll(const Detail::ShiftType st, const Detail::ShiftPrint sp, const T shift)
        {
            if (st == Detail::ShiftType::Left) PushBack(sp.After, shift);
        }
