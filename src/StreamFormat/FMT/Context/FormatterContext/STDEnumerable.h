...
#include "StreamFormat/FMT/Context/Utils/ContextsFunctions.h"
...
namespace StreamFormat::FMT
{
    ...
    {
    public:
        explicit inline STDEnumerable(const T& value, const std::basic_string_view<CharJoin>& strJoin = ", ", const std::basic_string_view<CharBegin>& strBegin = "{ ",
                                      const std::basic_string_view<CharEnd>& strEnd = " }", const Detail::DataType beginIdx = 0,
    ...
    struct FormatterType<STDEnumerable<T, CharBegin, CharJoin, CharEnd>, FormatterContext>
    {
        static void Format(const STDEnumerable<T, CharBegin, CharJoin, CharEnd>& enumerable, FormatterContext& context)
        {
            ...
            {
                // TODO: Why ? ...
                Detail::FunctionApplyNextOverride applyNextOverride(context);
                ...
                std::for_each_n(enumerable.GetValue().cbegin() + enumerable.GetBeginIdx(), enumerable.GetSize(), [&](const auto& element) {
                    if (first)
                        ...
                        context.BufferOut().WriteIndentStringView(enumerable.GetStrJoin());
    ...
    struct FormatterType<ForwardAsSTDEnumerable<T>, FormatterContext>
    {
        ...
        {
            STDEnumerable<T> enumerable(container, context.GetFormatData().GetSpecifierAsText("join", STDEnumerableUtility::DefaultJoin),
