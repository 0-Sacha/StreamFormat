...
namespace EngineCore::LoggerManager::Detail {
	...
	class XLogger : public Master
    {
        ...
        public:
            ...
            template<typename Format = std::string_view, typename ...Args>
            requires FMT::Detail::IsFmtConvertible<Format>::Value
            inline void Ok(const Format& format, Args&& ...args)      { return Log(LogStatus::OK, format, std::forward<Args>(args)...); }
            template<typename Format = std::string_view, typename ...Args>
            requires FMT::Detail::IsFmtConvertible<Format>::Value
            inline void Fail(const Format& format, Args&& ...args)    { return Log(LogStatus::FAIL, format, std::forward<Args>(args)...); }

            /////---------- NO-FORMAT Logger Status ----------/////
            template<typename T>
            inline void Ok(T&& t) { return Log(LogStatus::OK, std::forward<T>(t)); }
            template<typename T>
            inline void Fail(T&& t) { return Log(LogStatus::FAIL, std::forward<T>(t)); }

        public:
            template<typename Format = std::string_view, typename ...Args>
            requires FMT::Detail::IsFmtConvertible<Format>::Value
            inline void Basic(const Format& format, Args&& ...args)   { return Log(LogBasic{}, format, std::forward<Args>(args)...); }

            template<typename T>
            inline void Basic(T&& t)                                  { return Log(LogBasic{}, std::forward<T>(t)); }
	};
}
