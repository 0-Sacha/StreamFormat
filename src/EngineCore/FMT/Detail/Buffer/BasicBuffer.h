...
namespace EngineCore::FMT::Detail {
	...
	public:
		...
		void SetBuffer(const std::basic_string_view<CharBufferType>& buffer)						{ SetBuffer(buffer.data(), buffer.size()); }
		void SetCurrentPos(CharBuffer *const bufferCurrentPos)										{ m_CurrentPos = bufferCurrentPos; }
		inline void SetParentBufferForUpdate(BasicFormatterMemoryBuffer<CharBuffer>* parentBuffer)	{ m_ParentBuffer = parentBuffer; }
	...
	public:
		virtual bool RecoveryFunction(const std::size_t count)		{ return false; }

		// Format
		inline bool CanMoveForward()								{ if (m_CurrentPos < m_BufferEnd)			return true; return RecoveryFunction(1); }
		inline bool CanMoveForward(const std::size_t count)			{ if (m_CurrentPos + count < m_BufferEnd)	return true; return RecoveryFunction(count); }
		...
		inline void CanMoveForwardThrow(const std::size_t count)		{ if (CanMoveForward(count))	return; throw FormatBufferFull(); }
		inline void CanMoveBackwardThrow() const						{ if (CanMoveBackward())		return; throw FormatBufferIndexOutOfRange(); }
		inline void CanMoveBackwardThrow(const std::size_t count)		{ if (CanMoveBackward(count))	return; throw FormatBufferIndexOutOfRange(); }
